...
case "$1" in
  "create")
    ...
    docker-machine ssh npg-01 ETCD_ENDPOINTS=http://192.168.99.101:2379 sudo calicoctl node run --node-image=quay.io/calico/node:v2.6.8 --no-default-ippools
    docker-machine ssh npg-02 ETCD_ENDPOINTS=http://192.168.99.101:2379 sudo calicoctl node run --node-image=quay.io/calico/node:v2.6.8 --no-default-ippools
    docker-machine ssh npg-01 ETCD_ENDPOINTS=http://192.168.99.101:2379 calicoctl apply -f /vagrant/ipconfig.yml
    ;;
  ...
  "test")
    ...
    docker-machine ssh npg-01 calicoctl get pool
    echo ""
    echo "npg-02"
    echo "------"
    ...
    docker-machine ssh npg-02 ETCD_ENDPOINTS=http://192.168.99.101:2379 calicoctl get pool
    ;;
